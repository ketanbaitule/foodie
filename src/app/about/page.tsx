  ...
  return (
    <div className="flex flex-col justify-center items-center gap-8">
        ...
        <div className="grid grid-cols-11 gap-3 max-sm:gap-2 px-5">
          {about.map((src, index) => {
            return (
              <div
                key={index}
                className={`w-14 h-72 max-lg:w-9 max-sm:w-7 max-[480px]:w-6 relative col-span-1 shadow-[0_4px_16px_-1px_rgb(0,0,0,0.1),0_2px_4px_-2px_rgb(0,0,0,0.1)] shadow-slate-500 ${index === 0 ? "rounded-l-xl" : ""} ${index === about.length - 1 ? "rounded-r-xl" : ""}`}
              >
                <Image
                  src={src}
      ...
      <div className="flex flex-col md:flex-row items-center justify-around w-full mt-5 p-10 gap-y-5">
        <div className="relative flex-1 max-md:order-2">
          <Image
            src="/about-story.png"
        ...
        <div className="flex flex-1 flex-col items-start gap-5 md:ml-10 mt-8 max-md:order-1">
          <SecondaryHeading>Our Story</SecondaryHeading>
          <p className="text-xl md:text-2xl pl-2">
            Founded in 2023, Foodie began as a small family-owned seafood
            restaurant. Over the years, we&apos;ve expanded our menu and our
            reach, always maintaining our commitment to quality and flavor.
          </p>
        </div>
      </div>
